...
# ╔═╡ cd1f976e-2a89-11ee-09ed-87130798e2b7
using CUDA

# ╔═╡ 10fcff9d-ee62-4ff0-9b60-c253a8b139d3
# load packages
using Random: shuffle!

# ╔═╡ e94916ac-f2fc-44de-9fa2-d9deb54be3bb
using Images

# ╔═╡ dce3da33-a4a1-4dec-99c2-a238f63d6561
using Flux

# ╔═╡ 93153d00-872e-475f-8e54-a7feeab7dc44
using Flux: update!

# ╔═╡ 9e2c050b-856c-4cc8-9763-8c232f69fb59
using DataAugmentation

# ╔═╡ 1795c0df-a87d-4601-9a8d-57b1eb3504ec
using Metalhead, cuDNN

# ╔═╡ cc533eab-44d8-457c-a3cd-97d7b3890aca
import Base

# ╔═╡ 55dfaa16-0633-4b26-aa0e-2b0850bdb846
device = CUDA.functional() ? gpu : cpu

# ╔═╡ 724235de-d2f5-4ded-8253-6b83824f7aa0
# device = cpu

## Custom DataLoader
const CATS = readdir(abspath(joinpath("data", "animals", "cats")), join = true)

# ╔═╡ 25cb7ba5-6826-4ab7-8ad9-4d6914ed1195
const DOGS = readdir(abspath(joinpath("data", "animals", "dogs")), join = true)

# ╔═╡ 374ca73b-6dc9-4fc5-99c4-ea7e28ab0970
const PANDA = readdir(abspath(joinpath("data", "animals", "panda")), join = true)

# ╔═╡ 5c9deace-eb14-4283-acca-a93b56595165
struct ImageContainer{T<:Vector}
    img::T
end

# ╔═╡ 6d1011ba-b2c3-400b-ac8a-6ee6e0079680
imgs = [CATS..., DOGS..., PANDA...]

# ╔═╡ c561d8b0-1106-4a8e-abe8-c63769062d8b
shuffle!(imgs)

# ╔═╡ 854f54da-3222-449a-aaab-fbf5722278c3
data = ImageContainer(imgs)

# ╔═╡ d8567fd6-73ef-4b7f-abf3-fc5671d6e1fb
length(data::ImageContainer) = length(data.img)

# ╔═╡ 79480795-2bd3-498b-9bf0-b85cd8249229
const im_size = (224, 224)

# ╔═╡ 42449497-00a4-4087-8cb0-a0b2e7f1fbc1
tfm = DataAugmentation.compose(ScaleKeepAspect(im_size), CenterCrop(im_size))

# ╔═╡ 948cf713-f69b-403d-bf31-9b310d1fc630
name_to_idx = Dict{String,Int32}("cats" => 1, "dogs" => 2, "panda" => 3)

# ╔═╡ c7156d7c-2881-4d24-acc5-1714f0cd9681
const mu = [0.485f0, 0.456f0, 0.406f0]

# ╔═╡ 5c398060-4cc3-4cff-93c3-2feac5a739f1
const sigma = [0.229f0, 0.224f0, 0.225f0]

# ╔═╡ b4cf6864-ed3f-467d-b411-63a0ed5312e0
...
# ╔═╡ d3e1feb1-820a-4d20-bd23-17c4d30bd84a
# define DataLoaders
const batchsize = 16

# ╔═╡ 79a267cb-7d28-4bbd-809c-d975fbc7b388
ktrain = Flux.DataLoader(
...
# ╔═╡ e3f490ad-b26f-4dac-8491-f869e4a1d891
...
# ╔═╡ 3396786d-e9f3-4cfe-8cfa-4eb68c43b9a7
# Fine-tune | 🐇 mode
# define models 
m_infer = deepcopy(m[1]) |> device

# ╔═╡ 1692580d-3fed-4519-8fc9-ebeaf25a8dd6
m_tune = Chain(AdaptiveMeanPool((1, 1)), Flux.flatten, Dense(512 => 3)) |> device

# ╔═╡ 24728f3f-e45f-4710-b0f0-5829c9721063
...
# ╔═╡ 253714c8-fa66-4c6c-976a-f9535c6c2be7
...
# ╔═╡ e634a595-659a-4b74-bbe1-fd39f37cb3a1
for iter = 1:5
...
# ╔═╡ 7b1886c5-8028-4816-9d55-beaf913b60e8
# training loop
for iter = 1:5
...
# ╔═╡ dfc29c46-0148-47a7-9e46-42cb16fd9ab4
opt = Flux.setup(Flux.Optimisers.Adam(1e-3), m_tune);

# ╔═╡ ddab174a-a175-4712-994c-09617cfec5c7
...
# ╔═╡ e497d566-9375-4211-b7ef-b6ce71526fa2
device == gpu ? deval = Flux.CuIterator(deval) : nothing

# ╔═╡ bb98c4dd-1bbf-4c35-b18a-b6b607380099
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "083c95606adf7c356f8df356a7958b464a49253c"
...
git-tree-sha1 = "9073179282095c1ce9590e19525ad864a7b83211"
